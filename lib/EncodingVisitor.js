...
class EncodingVisitor extends Visitor {
  ...
  encodeNestedStructure(object, schema, currentKeyPath = '') {
    ...
    this.appendVisitorStream(nestedVisitor);
  }
  encodeCollection(collection, schema, currentKeyPath = '') {
    this.stream.writeUInt16(collection.length);
    const itemVisitor = this.createVisitorInstance(); // Create a single instance for reuse
    ...
    for (let i = 0; i < collection.length; i++) {
      ...
      this.appendVisitorStream(itemVisitor); // Append the stream from the reused visitor
      itemVisitor.resetStreamOffset(); // Reset the offset for the next item
    }
  }
  appendVisitorStream(visitor) {
    const bitsToAppend = visitor.stream.offset;
    for (let i = 0; i < bitsToAppend; i++) {
      const bit = visitor.stream.bitBuffer._getBit(i);
